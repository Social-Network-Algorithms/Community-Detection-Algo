...
class MongoFriendGetter(FriendGetter):
    ...
    def download_missing_friends(self, user_id: str):
        ...
        _, friends_user_ids = bluesky_getter.get_friends_ids_by_user_id(user_id, None)
    ...
    def get_user_friends_ids(self, user_id: str) -> List[str]:
        ...
        if doc is None:
            self.download_missing_friends(user_id)
            doc = self.friend_collection.find_one({"user_id":str(user_id)})
